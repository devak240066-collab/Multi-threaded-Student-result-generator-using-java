...
/**
 * Small embedded HTTP server exposing a front page to submit CSV and optionally persist results to MySQL.
 */
public class WebApp {
    ...
    static class SubmitHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            String dbPass = params.get("dbPass");
            ...
            try {
                ...
                if (dbUrl != null && !dbUrl.trim().isEmpty()) {
                    try {
                        DB.persist(dbUrl.trim(), dbUser == null ? "" : dbUser.trim(), dbPass == null ? "" : dbPass.trim(), results, ds.subjects);
                        out.append("<p>Saved results to database.</p>");
                    } catch (SQLException sqe) {
                        out.append("<p style='color:red'>Failed to save to DB: " + escapeHtml(sqe.getMessage()) + "</p>");
                    }
                }
    ...
    static class ApiExportHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            Map<String,String> params = parseFormStatic(body);
            String csv = params.getOrDefault("csvData", "");
            String dbUrl = params.getOrDefault("dbUrl", "results.xlsx");
            if (csv.trim().isEmpty()) { sendJson(exchange, "{\"ok\":false,\"error\":\"No CSV provided\"}"); return; }
            try {
                DataSet ds = DataLoader.fromCsvString(csv);
                List<StudentResult> results = computeParallel(ds, Math.max(2, Runtime.getRuntime().availableProcessors()), 40);
                rankResults(results);
                try { DB.persist(dbUrl.trim(), "", "", results, ds.subjects); sendJson(exchange, "{\"ok\":true}"); }
                catch (SQLException sqe) { sendJson(exchange, "{\"ok\":false,\"error\":\""+escapeJson(sqe.getMessage())+"\"}"); }
            } catch (Exception e) {
